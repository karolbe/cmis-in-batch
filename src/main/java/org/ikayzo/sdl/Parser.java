...
class Parser {
	...
	private List<Token> physicalLine;
	private int physicalTokenStart; 
		...
		if(physicalTokenStart==0) {
			...
			for(;pos<lineLength; pos++) {
				...
				if(c=='"') {	
					...
					if(firstLogicalLineSize==-1)
						firstLogicalLineSize=toks.size();
					sb=null;
				} else if("{}=:".indexOf(c)!=-1) {
					// handle punctuation
			...
			if(firstLogicalLineSize==-1) {
				...
				physicalLine = toks;
				physicalTokenStart = firstLogicalLineSize;
				return toks.subList(0, firstLogicalLineSize-1);
			}
		} else {
			List<Token> logicalLine = new ArrayList<Token>();
			int size = physicalLine.size();
			boolean foundEnd = false;
			
			for(int i=physicalTokenStart; i<size; i++) {
				Token t = physicalLine.get(i);
				if(t.type==Type.SEMICOLON) {
					physicalTokenStart=i+1;
					foundEnd = true;
					break;
				} else {
					logicalLine.add(t);
			...
			if(!foundEnd) {
				// we reached the end of the physical line
				physicalTokenStart=0;
			}
	...
	enum Type {
		...
		// punctuation
		COLON, EQUALS, START_BLOCK, END_BLOCK, SEMICOLON,
